...
//		new Effect.BlindUp('messages',{fps:25,duration: 0.125});
    ...
    // displays any messages that have been returned.
		...
		new Ajax.Request('/js/take_turn/',{onComplete:function(t) {
				...
				var response = t.responseText.replace(match, '');
				...
				render_changes();
				delete_offscreen_squares();

			total_dice+=dice_roll;
//			alert("total: " + total_dice + ", just rolled: " + dice_roll);
			if (total_dice<9)
				{
				move_car(dice_roll);
				}
			...
			{
				cursor_up(total_dice);
				move_board(total_dice);	
//				$('buildings_pane').innerHTML = views[1];
//				new_buildings = views[1];
//				new_buildings = crappy_render_full_map();
